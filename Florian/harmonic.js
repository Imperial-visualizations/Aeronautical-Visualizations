...
let physics = (function() {
    ...
    function newPosition() { // gets values from the sliders in the HTML (value is called by the ID of the slider in the HTML file)
        k_ani = document.getElementById("Spring").value;
        c_ani = document.getElementById("Damping").value;
        m_ani = document.getElementById("Mass").value;
        // Initial displacement and velocity
        v_0_ani = document.getElementById("IniVelo").value;
        ...
        t_ani += interval_ani; //time counter, incremented at each iteration
        ...
        let D= 0;
        let d_array=[];
        KS=0.5*k_ani*Math.pow(r_ani,2);
        T=0.5*m_ani*Math.pow(v_ani,2);

        D=D[i-1]+ c*v[i]*(r[i]-r[i-1]);
        console.log(D);
        d_array.push(D);
