...
// Energy calculations
    for (let i = 1; i < n; i++) {
        KS[0]=0.5* k *Math.pow(r_0,2);
        KE[0]=0.5* m *Math.pow(v_0,2);
        D[0]=c*v_0*r_0;
        KS[i] = 0.5 * k * Math.pow(r[i], 2); //strain energy of the spring
        KE[i] = 0.5 * m * Math.pow(v[i], 2); // kinetic energy of the mass
        D[i]=D[i-1]+c*v[i]*(r[i]-r[i-1]); // Energy dissipated by damping
